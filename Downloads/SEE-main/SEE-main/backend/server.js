...
app.post('/api/state/save', async (req, res) => {
    ...
    try {
        console.log(`[Server] Save request for ${userId}. Keys:`, Object.keys(req.body));
        console.log(`[Server] Resources in body type:`, typeof resources);
        console.log(`[Server] Is Array?`, Array.isArray(resources));
        if (resources === undefined) console.log(`[Server] Resources is UNDEFINED`);
        if (resources === null) console.log(`[Server] Resources is NULL`);
        ...
        */
        ...
        // save resources
        // Only update resources if provided in the payload AND is an array.
        // This prevents overwriting new recommendations when only updating profile/skills.
        if (Array.isArray(resources)) {
            ...
            // If resources is undefined or null, we do NOTHING. preserving existing data.
            if (resources !== undefined) console.log("[Server] Resources update skipped (not an array). val:", resources);
        }
...
app.post('/api/resources/recommend', async (req, res) => {
    ...
    try {
        ...
        // Get receiver name for 'Sent to' note
        const receiverRes = await db.query('SELECT name, username FROM users WHERE id=$1', [receiverId]);
        const receiverName = receiverRes.rows[0].name || receiverRes.rows[0].username;

        // 1. Insert into RECEIVER'S list (The Recommendation)
        await db.query(
            'INSERT INTO resources (user_id, skill, title, url, note, author, peer_index) VALUES ($1, $2, $3, $4, $5, $6, $7)',
            [receiverId, resource.skill, resource.title, resource.url, resource.note, senderName, -1]
        );

        // 2. Insert into SENDER'S list (The Record)
        // We append "Sent to [Name]" to the note so they recall.
        ...
        await db.query(
            ...
            // peer_index 0 (or null) to treat as normal item, Author "You"
        );
